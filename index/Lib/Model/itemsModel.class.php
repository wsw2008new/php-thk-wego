...
class itemsModel extends RelationModel
{
    ...
    'items_tags' => array(
     ...
     'user'=>array(
            'mapping_type'  => BELONGS_TO,    /*user和item是1对多的关系 一个用户可以发布多个商品*/
            'class_name'    => 'user',
            'foreign_key'   => 'uid',
	),
	);
	...
	function del($id){
		...
		$res=$mod->where("id=$id and uid=".$_COOKIE['user']['id'])->find();
		if($res){
			$cate_mod->where('id='.$res['cid'])->setDec('item_nums');
			return $mod->where("id=$id and uid=".$_COOKIE['user']['id'])->delete();
		}
