...
class flinkAction extends baseAction {
	...
	function index() {
		...
		$p = new Page($count, 20);
		$list = $link_mod->where($where)->field($prex . 'items_comments.*,' . $prex . 'items.title as title')->join('LEFT JOIN ' . $prex . 'items ON ' . $prex . 'items_comments.items_id = ' . $prex . 'items.id ')->limit($p->firstRow . ',' . $p->listRows)->order($prex . 'items_comments.add_time DESC')->select();
