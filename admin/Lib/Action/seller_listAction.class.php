<?php
class seller_listAction extends baseAction
{	//显示列表
	...
	public function edit()
	{
		...
		$cate_list_mod = D('seller_cate');
		$cate_list=$cate_list_mod->where("status=1")->order("sort DESC")->select();	
		$cate_list=setArrayFormItem($cate_list,'id','name');  //所有分类
		//选中分类
		$seller_list_cate_mod = D('seller_list_cate');
	...
	private function collect_success($message, $jump_url, $dialog='')
	{
		...
		$this->display(APP_PATH.'Tpl/'.C('DEFAULT_THEME').'/seller_list/collect_success.html');
		exit;
	}
